          ...
          <View key={g.title}>
            ...
            {g.items.map(it => (
              <Pressable key={it.route} accessibilityRole="button" style={({ pressed }) => [styles.row, rtlRow(rtl), pressed && styles.rowPressed]} onPress={() => navigation.navigate(it.route)}>
                <View style={styles.icon}><Ionicons name={it.icon} size={19} color={colors.primary} /></View>
                <View style={{ flex: 1 }}>
...
const makeStyles = (colors) => StyleSheet.create({
  ...
  icon: { width: 40, height: 40, borderRadius: 12, alignItems: 'center', justifyContent: 'center', backgroundColor: colors.primary + '14' },
